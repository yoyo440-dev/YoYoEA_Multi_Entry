...
datetime      g_lastLoggedExitTime = 0;
...
//+------------------------------------------------------------------+
//| Utility: parse timestamp text from log                           |
//+------------------------------------------------------------------+
datetime ParseLogTimestamp(const string value)
  {
   string trimmed = StripUtf8Bom(TrimString(value));
...
//+------------------------------------------------------------------+
//| Utility: track ticket in current exit cache                      |
//+------------------------------------------------------------------+
bool IsTicketTracked(const int ticket)
  {
...
void TrackExitTicket(const int ticket)
  {
   ...
   while(!FileIsEnding(handle))
     {
      ...
      if(StringLen(eventValue) == 0 || eventValue != "EXIT")
         continue;
      ...
      string timestampText = (ArraySize(rowFields) > 0 ? rowFields[0] : "");
      datetime eventTime = ParseLogTimestamp(timestampText);

      if(eventTime > g_lastLoggedExitTime)
        {
         g_lastLoggedExitTime = eventTime;
         ArrayResize(g_exitLoggedTickets, 0);
        }

      if(eventTime == g_lastLoggedExitTime && ticket != 0)
...
//+------------------------------------------------------------------+
bool IsExitLogged(const int ticket, const datetime closeTime)
  {
   ...
   return(IsTicketTracked(ticket));
  }

//+------------------------------------------------------------------+
//| Utility: mark exit ticket as logged                              |
//+------------------------------------------------------------------+
void MarkExitLogged(const int ticket, const datetime closeTime)
  {
   if(closeTime > g_lastLoggedExitTime)
     {
      g_lastLoggedExitTime = closeTime;
      ArrayResize(g_exitLoggedTickets, 0);
     }

   if(closeTime == g_lastLoggedExitTime && ticket != 0)
      TrackExitTicket(ticket);
  }
   ...
   for(int i = total - 1; i >= 0; i--)
     {
      ...
      MarkExitLogged(ticket, closeTime);
